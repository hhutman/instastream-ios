...
- (void)captureOutput:(nullable LFVideoCapture *)capture pixelBuffer:(nullable CVPixelBufferRef)pixelBuffer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.delegate && [self.delegate respondsToSelector:@selector(liveSessionCapturedFrame:)]) {
            [self.delegate liveSessionCapturedFrame:self];
        }
    });
}
...
- (GPUImageVideoCamera *)sourceFilter {
    GPUImageVideoCamera *v = self.videoCaptureSource.sourceFilter;
    NSLog(@"v = %@", v);
    return v;
}
- (GPUImageFilter *) sinkFilter {
    return _videoCaptureSource.sinkFilter;
}
- (BOOL) useCustomFilters {
    return _videoCaptureSource.useCustomFilters;
}

-(void)setUseCustomFilters:(BOOL)customFilters {
    [self.videoCaptureSource setUseCustomFilters:customFilters];
}
